...
class AutoOverLord(ZergUnit):
    ...
    async def overlord_count_calc(self) -> int:
        growth_speed = self.income_calculator.mineral_income / 20

        build_time = 8  # overlord build time
        larva = self.cache.own(UnitTypeId.LARVA).amount
        bonus = min(larva * 3, int((self.ai.minerals - 100) / 20))
        ...
        return ceil((predicted_supply - self.ai.supply_cap) / 10) + current_overlords
