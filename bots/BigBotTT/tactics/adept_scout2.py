...
from tactics.micro import MicroAdeptsHarass


class AdeptScout2(ActBase):
    ...
    async def start(self, knowledge: "Knowledge"):
        ...
        self.micro.generic_micro = MicroAdeptsHarass(False)
    ...
    async def micro_adepts(self, adepts: Units):
        ...
        for adept in adepts:  # type: Unit
            ...
            if shade_tag is not None:
                ...
                if self.cd_manager.is_ready(adept.tag, AbilityId.ADEPTPHASESHIFT_ADEPTPHASESHIFT, 6.9):
                    nearby_workers = (
                        self.cache.enemy_in_range(shade.position, 10).of_type(self.unit_values.enemy_worker_type).amount
                        > 0
                    )
                    # Determine whether to cancel the shade or not
                    if not nearby_workers and self.enemy_units_manager.danger_value(
                        adept, adept.position
                    ) + 3 < self.enemy_units_manager.danger_value(adept, shade.position):
            ...
            if self.cd_manager.is_ready(adept.tag, AbilityId.ADEPTPHASESHIFT_ADEPTPHASESHIFT) and (
                shade_distance < 11 or shade_distance > 30 or self.cache.enemy_in_range(center, 8).amount > 0
            ):
        ...
        self.combat.execute(self.target_position, MoveType.Harass, rules=self.micro)
