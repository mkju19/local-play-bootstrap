...
taunt_conditions = {
    ...
    TauntType.RushDetected: lambda k: k.ai.build_detector.rush_detected,
    TauntType.Victory: lambda k: k.ai.game_analyzer.predicting_victory,
    TauntType.Defeat: lambda k: k.ai.game_analyzer.predicting_defeat,
...
taunts = {
    ...
    TauntType.RushDetected: ["Do you need a knife for that cheese?", "Don't you run with scissors"],
    TauntType.Victory: [
...
class TauntEnemy(ActBase):
    def __init__(self):
        self.is_chat_allowed = False
        super().__init__()

    async def start(self, knowledge: "Knowledge"):
        await super().start(knowledge)
        self.is_chat_allowed = self.knowledge.get_boolean_setting("general.chat")
        self.chat_manager = self.knowledge.get_required_manager(ChatManager)
    ...
    async def execute(self) -> bool:
        ...
        for taunt_type in TauntType:
            if taunt_conditions.get(taunt_type)(self.knowledge):
                ...
                await self.chat_manager.chat_taunt_once(str(taunt_type), lambda: random.choice(taunt_list))
